...
import argparse
"""
实时检测电池绝缘异常，基于滑动窗口判定绝缘阻值异常并产生二级告警
:return: 绝缘异常告警结果存入Hive表
"""
# 创建SparkSession, 作为读取数据、处理源数据、配置回话和管理集群资源的入口
spark = SparkSession.builder \
    .appName("AnalysingInsulationAlarmsLv2") \
...
ap19_insulation_resistance_threshold = [8000, 40]  # kΩ
ap19_soc_charge_start_array = [1, 2]
ap19_Realtime_continually_TSP_threshold = [4, 4]  # 帧
ap19_Realtime_frame_TSP_threshold = 3  # 帧
ap19_current_threshold = 1  # A
ap19_sample_bin_threshold = [10, 5]
ap19_sample_abnormal_threshold = [30, 0.7, 0.5, 0.9, 7, 0.5]

ap19_insulation_threshold_1 = float(ap19_insulation_resistance_threshold[0])
ap19_insulation_threshold_2 = float(ap19_insulation_resistance_threshold[1])
ap19_continue_threshold_1 = int(ap19_Realtime_continually_TSP_threshold[0])
ap19_continue_threshold_2 = int(ap19_Realtime_continually_TSP_threshold[1])
ap19_frame_threshold = int(ap19_Realtime_frame_TSP_threshold)
...
df_detail = df_detail.filter(f"insulation_resistance < {ap19_insulation_threshold_1}")

# Step2: 充电状态与继电器判定
# 排除充电连接状态 (charge_state in [1,2])
df_detail = df_detail.filter(
    ~col("charge_state").isin(ap19_soc_charge_start_array)
)
...
df_detail = df_detail.filter(
    ...
    (col("bms_cntctr_sts").isNull() & (functions.abs(col("current")) <= ap19_current_threshold))
)
...
df_detail = df_detail.withColumn("insulation_low_count_4", functions.sum(
    functions.expr(f"CASE WHEN insulation_resistance < {ap19_insulation_threshold_1} THEN 1 ELSE 0 END")
...
df_detail = df_detail.withColumn("window_pass", functions.expr(f"""
    CASE WHEN frame_count_4 >= {ap19_continue_threshold_1}
        AND diff_count_4 >= {ap19_frame_threshold}
        AND insulation_low_count_4 >= {ap19_continue_threshold_1}
    THEN 1 ELSE 0 END
"""))

# Step4: 严重绝缘异常判定 - 绝缘 < 40 kΩ 且持续4帧以上
df_detail = df_detail.withColumn("insulation_severe_count_4", functions.sum(
    functions.expr(f"CASE WHEN insulation_resistance < {ap19_insulation_threshold_2} THEN 1 ELSE 0 END")
).over(Window.partitionBy("battery_id").orderBy("sample_timestamp").rowsBetween(-3, 0)))

df_detail = df_detail.withColumn("is_alarm", functions.expr(f"""
    CASE WHEN window_pass = 1
        AND insulation_severe_count_4 >= {ap19_continue_threshold_2}
    THEN 1 ELSE 0 END
"""))
...
df_alarm = df_alarm.select(
 ...
 .withColumn("diagnosis_code", functions.lit("P0101")) \
 .withColumn("alarm_type", functions.lit("insulation_abnormal_lv2"))
